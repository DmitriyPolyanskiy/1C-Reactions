...
Процедура РКЦ_ПриОпределенииВидовПодключаемыхКоманд(ВидыПодключаемыхКоманд)
	
	Вид = ВидыПодключаемыхКоманд.Добавить();
	Вид.Имя         = "РКЦ_Реакции";
	Вид.ИмяПодменю  = "ПодменюОтреагировать";
	Вид.Заголовок   = НСтр("ru = 'Отреагировать'");
	Вид.Картинка    = БиблиотекаКартинок.РКЦ_Реакция;
	Вид.Отображение = ОтображениеКнопки.КартинкаИТекст;
	Вид.Порядок     = 80;
...
Процедура РКЦ_ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды)
	
	ТипПараметра = Новый Массив;
	Для Каждого Источник Из Источники.Строки Цикл
		Если РКЦ_Реакции.НастройкиРеакцийОбъекта(Источник.ПолноеИмя) Тогда
			ТипПараметра.Добавить(Источник.ТипСсылкиДанных);
		КонецЕсли;
	КонецЦикла;
	
	Если ТипПараметра.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	...
	Пока Реакция.Следующий() Цикл
		...
		ЗаполнитьОбщиеДанныеКоманды(КомандаРеакции, ТипПараметра);
		КомандаРеакции.Идентификатор = Реакция.Код;
		КомандаРеакции.Представление = Реакция.Наименование;
	...
	КомандаУдалить = Команды.Добавить();
	ЗаполнитьОбщиеДанныеКоманды(КомандаУдалить, ТипПараметра);
	...
	КомандаУдалить.Важность = "СмТакже"; // в самый конец
	КомандаУдалить.Обработчик = "РКЦ_РеакцииКлиент.УдалитьРеакцию";
	
КонецПроцедуры

Процедура ЗаполнитьОбщиеДанныеКоманды(КомандаРеакции, ТипПараметра)
	
	КомандаРеакции.ТипПараметра = Новый ОписаниеТипов(ТипПараметра);
